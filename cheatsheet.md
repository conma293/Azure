...
```
#### Administrative Unit
(Organisational Unit OU equiv for cloud) - _we canm get the ID of the administrative unit via API call for '/users/userx/memberOf' of a member user/group_
```
...
Check specific roles and user within the administrative unit (grab the ```RoleId```):
```
